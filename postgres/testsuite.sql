...
45	25	Cookie name prefixes	<p>The <em>__Secure-</em>, <em>__Host-</em>, <em>__Http</em>, and <em>__Host-Http-</em> cookie name prefixes let a server signal that a cookie must satisfy extra security constraints, and instruct a compliant browser to reject the cookie if those constraints are not met. A cookie whose name begins with <em>__Secure-</em> must carry the <em>Secure</em> attribute and be set over HTTPS. A cookie whose name begins with <em>__Host-</em> additionally must not carry a <em>Domain</em> attribute and must have its <em>Path</em> set to /, which locks the cookie to the exact host that set it and helps defend against cookie injection from subdomains. A cookie with the <em>__Http-</em> prefix must be <em>Secure</em> and <em>HttpOnly</em>, which is used to ensure the cookie has been set by the Set-Cookie header, not JavaScript. The <em>__Host-Http-</em> prefix combines the constraints of the <em>__Host-</em> and <em>__Http-</em> prefixes. <p>The <em>__Secure-</em> and <em>__Host-</em> prefixes are supported by all major browsers, but <em>__Http-</em> and <em>__Host-Http-</em> are still new and may not be supported. </p> </p>	4	t	t	\N
46	30	Cross-Origin Resource Policy	<p> In this category of tests, we test the browser’s implementation of <em>Cross-Origin-Resource-Policy</em> (CORP) as specified by the <a target="_blank" href="https://fetch.spec.whatwg.org/#http-cross-origin-resource-policy" >Fetch Standard</a>. The Cross-Origin-Resource-Policy header is an HTTP response header sent by the resource’s server, when a HTTP request is made, it is used to declare which documents are permitted to load the subresource. The browser then enforces this when the server responds, depending on the origin of the page and the policy sent by the server in the response header. This mechanism was introduced to prevent sensitive resources from being embedded by malicious cross-origin documents, which could otherwise read their contents through side-channel attacks such as Spectre. </p>	5	t	t	\N
47	11	nonces	<p>These tests check the mechanism introduced in Content Security Policy Level 2 where script and style elements can be whitelisted by their assigned nonces.</p>	18	t	t	\N
48	11	hashes	<p>Content Security Policy Level 2 also allows an inline <span class="tt">&lt;script&gt;</span> or <span class="tt">&lt;style&gt;</span> element to be allowed by a cryptographic hash of its exact contents. The server computes a SHA-256, SHA-384 or SHA-512 digest of the inline code and places it in the header as <span class="tt">'sha256-...'</span>, <span class="tt">'sha384-...'</span> or <span class="tt">'sha512-...'</span>. A compliant browser hashes the inline content itself and executes it only if the digest matches one in the policy, so any injected inline code with a different hash is blocked. As with nonces, when a hash source is present the browser ignores <span class="tt">'unsafe-inline'</span> for that resource type. The digest must cover the exact bytes of the element's contents, including whitespace.</p>	19	t	t	\N
49	11	unsafe-hashes	<p>The <em>unsafe-hashes</em> source expression was introduced in Content Security Policy Level 3, it is used to extend the capabilities of hash based CSP directives, allowing inline event handler scripts (e.g. onload), javascript: URLs, and inline style attributes to be whitelisted by their hashes. By default hash based CSP directives only apply to <em>script</em> and <em>style</em> elements.</p>	20	t	t	\N
50	11	strict-dynamic	<p>The <span class="tt">'strict-dynamic'</span> source expression was introduced in Content Security Policy Level 3. When it is present in <span class="tt">script-src</span>, a script that is already trusted by either a nonce or a hash is permitted to dynamically generatr more non parser inserted script elements. This lets a policy rely on nonces and hashes without having to maintain a host allowlist. </p>	21	t	t	\N
\.
...
475	SameSite=Strict cookie not sent cross-site	\N	block	critical	44	1	sameSiteCookieTest(475, true, "strict")	t
476	SameSite=Lax cookie not sent cross-site subresource	\N	block	critical	44	2	sameSiteCookieTest(476, true, "lax")	t
477	SameSite=None without Secure is ignored	\N	block	warning	44	3	sameSiteCookieTest(477, true, "none-insecure")	t
478	SameSite=None with Secure sent cross-site	\N	allow	warning	44	4	sameSiteCookieTest(478, false, "none")	t
479	Cookie without SameSite attribute defaults to Lax (not sent cross-site)	\N	block	warning	44	5	sameSiteCookieTest(479, true, "default")	t
...
486	__Http- cookie with Secure and HttpOnly is accepted	\N	allow	warning	45	7	cookiePrefixTest(486, "http-ok", true)	t
...
502	external script with matching nonce with script-src 'nonce-...'	\N	allow	warning	47	1	cspTest(502, 274, "default-src 'none'; script-src 'nonce-correct-nonce'", false, {})	t
503	external script with no nonce with script-src 'nonce-...'	\N	block	warning	47	2	cspTest(503, 275, "default-src 'none'; script-src 'nonce-correct-nonce'", true, {})	t
504	external script with mismatched nonce with script-src 'nonce-...'	\N	block	warning	47	3	cspTest(504, 276, "default-src 'none'; script-src 'nonce-correct-nonce'", true, {})	t
505	inline script with matching nonce with script-src 'nonce-...'	\N	allow	warning	47	4	cspTest(505, 277, "default-src 'none'; script-src 'nonce-correct-nonce'", false, {})	t
506	inline script with no nonce with script-src 'nonce-...'	\N	block	warning	47	5	cspTest(506, 278, "default-src 'none'; script-src 'nonce-correct-nonce'", true, {})	t
507	inline script with no nonce with script-src 'unsafe-inline' 'nonce-...'	\N	block	warning	47	6	cspTest(507, 279, "default-src 'none'; script-src 'unsafe-inline' 'nonce-correct-nonce'", true, {})	t
508	link stylesheet with matching nonce with style-src 'nonce-...'	\N	allow	warning	47	7	cspTest(508, 280, "default-src 'none'; style-src 'nonce-correct-nonce'", false, {})	t
509	link stylesheet with no nonce with style-src 'nonce-...'	\N	block	warning	47	8	cspTest(509, 281, "default-src 'none'; style-src 'nonce-correct-nonce'", true, {})	t
510	link stylesheet with mismatched nonce with style-src 'nonce-...'	\N	block	warning	47	9	cspTest(510, 282, "default-src 'none'; style-src 'nonce-correct-nonce'", true, {})	t
511	inline <style> with matching nonce with style-src 'nonce-...'	300	allow	warning	47	10	cspTest(511, 283, "default-src 'none'; style-src 'nonce-correct-nonce'; img-src 'self'", false, { timeout: 300 })	t
512	inline <style> with no nonce with style-src 'nonce-...'	300	block	warning	47	11	cspTest(512, 284, "default-src 'none'; style-src 'nonce-correct-nonce'; img-src 'self'", true, { timeout: 300 })	t
513	inline <style> with no nonce with style-src 'unsafe-inline' 'nonce-...'	300	block	warning	47	12	cspTest(513, 285, "default-src 'none'; style-src 'unsafe-inline' 'nonce-correct-nonce'; img-src 'self'", true, { timeout: 300 })	t
514	inline script with matching sha256 hash with script-src 'sha256-...'	\N	allow	warning	48	1	cspTest(514, 286, "default-src 'none'; script-src 'sha256-{script}'", false, {})	t
515	inline script with non-matching sha256 hash with script-src 'sha256-...'	\N	block	warning	48	2	cspTest(515, 287, "default-src 'none'; script-src 'sha256-AHjPTQIXJaHRXq0qBw+J3E2milLHt2FTk4+95Q87zkA='", true, {})	t
516	inline script with matching sha384 hash with script-src 'sha384-...'	\N	allow	warning	48	3	cspTest(516, 288, "default-src 'none'; script-src 'sha384-{script}'", false, {})	t
517	inline script with matching sha512 hash with script-src 'sha512-...'	\N	allow	warning	48	4	cspTest(517, 289, "default-src 'none'; script-src 'sha512-{script}'", false, {})	t
