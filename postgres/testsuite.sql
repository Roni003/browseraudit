...
47	11	nonces	<p>A <em>nonce</em> ("number used once") is a random value the server places in the CSP header (e.g. <span class="tt">script-src 'nonce-abc123'</span>) and repeats as a <span class="tt">nonce</span> attribute on each <span class="tt">&lt;script&gt;</span> or <span class="tt">&lt;style&gt;</span> element it wishes to allow. A compliant browser executes only those elements whose nonce attribute matches the value in the header, blocking any injected markup that lacks the secret nonce. This is one of the CSP2/CSP3 mechanisms recommended for mitigating XSS without resorting to <span class="tt">'unsafe-inline'</span>. Note that when a nonce (or hash) is present, a compliant browser ignores <span class="tt">'unsafe-inline'</span> for that resource type.</p><p>For testing purposes we use a fixed nonce value; browsers do not verify that a nonce is unpredictable, only that the header and element values match.</p>	18	t	t	\N
\.
