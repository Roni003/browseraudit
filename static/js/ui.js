...
var browserAuditUI = (function () {
  ...
  var testReportSummaryStringFragments = {
    ...
    timeout:
      "Note that this result may be unreliable due to the test's use of a %TIMEOUT%ms timeout; your web browser may not have executed the test quickly enough, or your Internet connection may be too slow.",
    pass: "This test executed in &#x2248;%TIME%ms and <strong>passed</strong> for the following reason: <em>%REASON%</em>",
    warning:
      "This <strong>non-critical</strong> test executed in &#x2248;%TIME%ms and <strong>failed</strong> for the following reason: <em>%REASON%</em>",
    critical:
      "This <strong>critical</strong> test executed in &#x2248;%TIME%ms and <strong>failed</strong> for the following reason: <em>%REASON%</em>",
  ...
  var publicInterface = {
    ...
    categorySelectionPanel: {
      add: function (id, title, description, parent, inDefault) {
        var categoryElement = $("<tr>", {
          class: "treegrid-" + id + (parent !== null ? " treegrid-parent-" + parent : ""),
        }).append(
          $("<td>")
            .append(
              $("<div>", { class: "category-checkbox" }).append(
                $("<input>", {
                  type: "checkbox",
                  id: "browseraudit-selected-" + id,
                  "data-category": id,
                  "data-default-category": inDefault,
                })
                  ...
                  .change(function () {
                    ...
                    if (isChecked) {
                      // Checking this box satisfies the requirement to select
                      // some test categories, so if the "no categories selected"
                      // alert box is visible, hide it
                      $("#browseraudit-categories .alert-no-categories").slideUp(100);
                    }
            ...
            .append(
              $("<div>", { class: "title" })
                .append($("<label>", { for: "browseraudit-selected-" + id }).html(title))
                .append($("<div>").append($("<small>").html(description !== null ? description : ""))),
            ),
        );
        ...
        if (parent === null) {
          ...
          categoryTreeTable
            .find(".treegrid-parent-" + parent)
            .last()
            .after(categoryElement);
        }
      },

      // Sets the checked state of all categories in the category selection panel
      // (state=true => select all, state=false => select none)
      setAll: function (state) {
        categoryTreeTable.find("input[type='checkbox']").prop("indeterminate", false).prop("checked", state);
      },

      // Resets the selected categories to the ones that form the default test
      // suite
      setDefaults: function () {
        // Start by deselecting all categories
        browserAuditUI.categorySelectionPanel.setAll(false);

        // Now select the defaults
        categoryTreeTable.find("input[type='checkbox'][data-default-category='true']").prop("checked", true);

        // This will satisfy the requirement to select some test categories, so
        // if the "no categories selected" alert box is visible, hide it
        $("#browseraudit-categories .alert-no-categories").slideUp(100);
      },
    ...
    settingsPanel: {
      scrollTo: function (settingName) {
        ...
        $.scrollTo("#browseraudit-settings .form-group[data-setting='" + settingName + "']", 400);
      },
    },
    ...
    notificationBar: {
      ...
      setMessage: function (glyphicon, message, onclickFunction) {
        ...
        notificationBarContainerElement.slideUp(300, function () {
          ...
          if (glyphicon !== null) {
            notificationBarTextElement.append($("<span>", { class: "glyphicon glyphicon-" + glyphicon }));
          }
    ...
    scoreboard: {
      ...
      setOutcome: function (outcome, newValue) {
        ...
        scoreboardElements.progressBar.pass.css(
          "width",
          floorToPlaces((scoreboardTally.pass / scoreboardTally.total) * 100, 2) + "%",
        );
        scoreboardElements.progressBar.warning.css(
          "width",
          floorToPlaces((scoreboardTally.warning / scoreboardTally.total) * 100, 2) + "%",
        );
        scoreboardElements.progressBar.critical.css(
          "width",
          floorToPlaces((scoreboardTally.critical / scoreboardTally.total) * 100, 2) + "%",
        );
        scoreboardElements.progressBar.skip.css(
          "width",
          floorToPlaces((scoreboardTally.skip / scoreboardTally.total) * 100, 2) + "%",
        );
      },
    ...
    executionInfoPanel: {
      ...
      addInfo: function (title, value, valueDetail, valueLabels, description) {
        ...
        // the function)
        var infoElement = $("<div>", { class: "form-group" })
          .append($("<label>", { class: "col-sm-2 control-label" }).text(title))
          .append(
            $("<div>", { class: "col-sm-10" }).append($("<p>", { class: "value" }).append($("<span>").html(value))),
          );

        // If a valueDetail was supplied, append it to the value element
        if (valueDetail !== null) {
          infoElement.find("p.value").append($("<span>", { class: "small" }).html(valueDetail));
        }
        ...
        if ($.isArray(valueLabels)) {
          $.each(valueLabels, function (i, label) {
            infoElement.find("p.value").append(
              $("<span>", { class: "label label-default" })
                .append(label[0] === null ? null : $("<img>", { src: label[0], role: "presentation" }))
                .append($("<span>").text(label[1])),
            );
          });
        }
        ...
        // element
        if (description !== null) {
          infoElement.find("div.col-sm-10").append($("<p>", { class: "description" }).html(description));
        }
      ...
      ready: function () {
        ...
        var uaLabels = $.grep(
          [
            ...
            $.grep([$.ua.browser.name, $.ua.browser.version], function (e, i) {
              return e !== undefined;
            }).join(" "),
            // OS name and version:
            $.grep([$.ua.os.name, $.ua.os.version], function (e, i) {
              return e !== undefined;
            }).join(" "),
          ],
          function (e, i) {
        ...
        $.each(uaLabels, function (i, label) {
          $("tt.useragent")
            .parent()
            .parent()
            .append($("<span>", { class: "label label-default" }).append($("<span>").text(label)));
        });
      },
    },
    ...
    testReportCategory: function (id, title, description) {
      var domElement = $("<div>", { class: "panel panel-default" })
        .append(
          $("<div>", { class: "panel-heading" }).append(
            $("<h4>", { class: "panel-title" })
              .append(
                $("<a>", {
                  href: "#browseraudit-category-" + id,
                  "data-toggle": "collapse",
                  "aria-controls": "browseraudit-category-" + id,
                }).text(title),
              )
              .append(
              ...
              .append($("<span>", { class: "badge badge-info pull-right hide" }))
              .append($("<span>", { class: "badge badge-warning pull-right hide" }))
              .append($("<span>", { class: "badge badge-danger pull-right hide" })),
          ),
        )
        .append(
          $("<div>", { class: "panel-collapse collapse in", id: "browseraudit-category-" + id }).append(
            $("<div>", { class: "panel-body" })
              .append($("<div>", { class: "category-description" }))
              .append($("<div>", { class: "category-children" })),
          ),
        );
      ...
      if (typeof description === "string" && description.length > 0) {
        domElement.find("div.category-description").addClass("category-description-present").html(description);
      }
      ...
      return {
        ...
        setOutcome: function (outcome, count) {
          outcomeElements[outcome]
            ...
            .attr(
              "title",
              categoryOutcomeStringFragments[outcome]
                .replace("%COUNT%", count)
                .replace("%S%", count === 1 ? "" : "s")
                .replace("%TENSE%", count === 1 ? "was" : "were"),
            );
        },
    ...
    testReport: function (id, title, behaviour, timeout, testFunction, reportData) {
      ...
      var domElement = $("<div>", { class: "list-group-item", id: "browseraudit-test-" + id })
        .append(
          $("<table>").append(
            $("<tr>")
              .append(
                $("<td>", { class: "nowrap" }).append(
                  $("<span>", { class: "label label-default" })
                    ...
                    .attr(
                      "title",
                      behaviour === "allow"
                        ? "This test relaxes a security policy and checks whether the web browser correctly allows a request that would usually violate the policy."
                        : "This test strengthens a security policy and checks whether the web browser correctly blocks a request that violates the policy.",
                    ),
                ),
              )
              .append(
                $("<td>", { class: "test-title" }).append(
                  $("<a>", {
                    class: "list-group-item-heading",
                    href: "#browseraudit-test-" + id + "-report",
                    "data-toggle": "collapse",
                    "aria-controls": "browseraudit-test-" + id + "-report",
                  }).text(title),
                ),
              )
              .append(
                $("<td>", { class: "nowrap" })
                  .append($("<span>", { class: "glyphicon glyphicon-time" }))
                  .append(
        ...
        .append(
          $("<div>", {
            class: "list-group-item-text well container-fluid collapse",
            id: "browseraudit-test-" + id + "-report",
          })
            .on("show.bs.collapse", function () {
              showTestReportData();
            })
            .on("hidden.bs.collapse", function () {
              reportDataContainer.empty();
            }),
        );
      ...
      var externalPreElement = function (reportDataComponent, dataType) {
        if (reportData.hasOwnProperty(reportDataComponent + "_content")) {
          var preElement = $("<pre>", { class: "pre-scrollable", tabindex: "0" }).append(
            $("<code>", { class: dataType }).html(
              beautify.hasOwnProperty(dataType)
                ? escapeHTMLEntities(beautify[dataType](reportData[reportDataComponent + "_content"]))
                : escapeHTMLEntities(reportData[reportDataComponent + "_content"]),
            ),
          );
          return preElement;
        } else {
      ...
      var reportDataComponents = {
        testFunction: {
          ...
          preElement: function () {
            var preElement = $("<pre>", { class: "pre-scrollable", tabindex: "0" }).append(
              $("<code>", { class: "js" }).html(escapeHTMLEntities(beautify.js(testFunction.toString()))),
            );
            return preElement;
          },
        },
        iframeSrc: {
          ...
          tooltip:
            "This test function dynamically creates an <code>&lt;iframe&gt;</code> as part of the test; this is its HTML source code.",
          preElement: function () {
            return externalPreElement("iframeSrc", "html");
          },
        },
        iframeHTTPHeader: {
          title: "Additional HTTP header served with <code>&lt;iframe&gt;</code>",
          tooltip:
            "The <code>&lt;iframe&gt;</code> dynamically created by this test function was additionally served with the following HTTP header.",
          preElement: function () {
            var preElement = $("<pre>", { class: "pre-scrollable", tabindex: "0" }).append(
              $("<code>", { class: "http" }).text(reportData.iframeHTTPHeader),
            );
            return preElement;
          },
        },
        innerIframeSrc: {
          ...
          tooltip:
            "The <code>&lt;iframe&gt;</code> dynamically created by this test function dynamically creates its own nested <code>&lt;iframe&gt;</code>; this is its HTML source code.",
          preElement: function () {
            return externalPreElement("innerIframeSrc", "html");
          },
        },
        stylesheetSrc: {
          title: "Stylesheet source code",
          tooltip:
            "The <code>&lt;iframe&gt;</code> dynamically created by this test function loads a dynamically-created external stylesheet; these are the CSS rules it contains.",
          preElement: function () {
            return externalPreElement("stylesheetSrc", "css");
          },
        },
        innerStylesheetSrc: {
          title: "Inner stylesheet source code",
          tooltip:
            "The dynamically-created external stylesheet loaded by this test function loads another dynamically-created external stylesheet of its own; these are the CSS rules it contains.",
          preElement: function () {
            return externalPreElement("innerStylesheetSrc", "css");
          },
        },
        ajaxHTTPHeader: {
          title: "Additional HTTP header served with AJAX response",
          tooltip:
            "The test function performed an AJAX request (e.g., via <code>$.ajax()</code> or <code>$.get()</code>), and the server's response to this request contained the following HTTP header.",
          preElement: function () {
            var preElement = $("<pre>", { class: "pre-scrollable", tabindex: "0" }).append(
              $("<code>", { class: "http" }).text(reportData.ajaxHTTPHeader),
            );
            return preElement;
          },
        },
        firstImageHTTPHeader: {
          ...
          tooltip:
            "The test function dynamically loaded images (i.e., by dynamically constructing <code>&lt;img&gt;</code> elements and inserting them into the page); the server's response to the <i>first</i> of these image-loading requests contained the following HTTP header.",
          preElement: function () {
            var preElement = $("<pre>", { class: "pre-scrollable", tabindex: "0" }).append(
              $("<code>", { class: "http" }).text(reportData.firstImageHTTPHeader),
            );
            return preElement;
          },
        },
        referrerPolicyHeader: {
          ...
          tooltip: "The test function dynamically loaded an image with the following Referrer Policy HTTP header.",
          preElement: function () {
            var preElement = $("<pre>", { class: "pre-scrollable", tabindex: "0" }).append(
              $("<code>", { class: "http" }).text(reportData.referrerPolicyHeader),
            );
            return preElement;
          },
        },
      ...
      var showTestReportData = function () {
        ...
        reportDataContainer.append(
          $("<div>", { class: "container-fluid" }).append(
            $("<div>", { class: "row" }).append(
              $("<div>", { class: "browseraudit-report-outcome-container col-md-12" }).append(
                $("<div>", { class: "alert alert-default" }).html(testReportSummaryStringFragments.executing),
              ),
            ),
          ),
        );
        ...
        $.each(
          ...
          function (k, reportDataComponent) {
                  ...
                  $("<div>", { class: "row" })
                    .append(
                      $("<div>", { class: "col-md-3" }).append(
                        $("<p>").html(reportDataComponents[reportDataComponent].title + ":"),
                      ),
                    )
                    .append(
                      $("<div>", { class: "col-md-9" }).append(reportDataComponents[reportDataComponent].preElement()),
                    ),
                );
            }
          },
      ...
      var updateTestReportData = function () {
        reportDataContainer
          .find("div.alert-default")
          .html(
            testReportSummaryStringFragments[testResult]
              .replace("%TIME%", testExecutionTime)
              .replace("%REASON%", testResultReason) +
              (testResult === "skip" || timeout === null
                ? ""
                : " " + testReportSummaryStringFragments.timeout.replace("%TIMEOUT%", timeout)),
          );
      };
      ...
      if (timeout === null) {
        domElement.find(".glyphicon-time").addClass("hide");
      } else {
        domElement
          .find(".glyphicon-time")
          .attr("rel", "tooltip")
      ...
      return {
        ...
        setResult: function (result, reason, executionTime) {
          if (result === "pass") {
            ...
            domElement.removeClass("list-group-item-warning list-group-item-danger list-group-item-info");
            ...
            domElement.removeClass("list-group-item-danger list-group-item-info").addClass("list-group-item-warning");
            ...
            domElement.removeClass("list-group-item-warning list-group-item-info").addClass("list-group-item-danger");
          } else {
            // result === "skip"
            resultIconElement
            ...
            domElement.removeClass("list-group-item-danger list-group-item-danger").addClass("list-group-item-info");
          }
...
// Make panel headings fully clickable
$(document).on("click.fullyClickablePanels", ".panel-heading, .panel-heading .panel-title", function (e) {
  // If the link in the panel header was clicked, stop executing this handler
  if (e.target != this) return false;

  $(this).find("a[data-toggle='collapse']").eq(0).trigger("click");
});
