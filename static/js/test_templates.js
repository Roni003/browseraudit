...
var parentChildSopTest = function (
  testID,
  shouldBeBlocked,
  fromParent,
  parentPrefix,
  parentDocumentDomain,
  childPrefix,
  childDocumentDomain,
) {
  ...
  if (!fromParent) {
    reportData.innerIframeSrc = childSrc
      .replace(/(test\.)?browseraudit\.(com|org)/, "browseraudit.com")
      .concat("?serveOnly=true");
  }
        ...
        .load(function () {
          $.get("/sop/result/" + testID, function (result) {
            if (result === "pass") {
              ...
              thisTest.CRITICAL(
                "The " + fromText + " was " + failBehaviourText + " to access the " + toText + "'s DOM.",
              );
            }
          });
        });
...
var domainScopeCookieTest = function (testID, shouldBeUnset, domainSetFrom, cookieDomain, domainAccessedFrom) {
  ...
  var path = "/";
  var img1path =
    "https://" + domainSetFrom + "/sop/cookie/" + name + "/" + timestamp + "/" + domain + "/" + $.base64.encode(path);
...
var cspTest = function (testID, cspID, policy, shouldBeBlocked, opts) {
  ...
  if ((200 <= cspID && cspID <= 211) || (220 <= cspID && cspID <= 229)) {
    reportData.innerIframeSrc =
      "/csp/serve/" + cspID + "/param-htmlb?policy=" + policyBase64 + "&defaultResult=" + defaultResult;
  ...
  if (cspID === 267) {
    reportData.ajaxHTTPHeader = `Report-To: {
    "group":"endpoint-1",
    "max_age":31536000,
    "endpoints":[{"url":"https://browseraudit.com/csp/pass/267/emptyhtml"}]
    }
Reporting-Endpoints: endpoint-1="https://browseraudit.com/csp/pass/267/emptyhtml"`;
  }
  ...
  if ((217 <= testID && testID <= 220) || (430 <= testID && testID <= 434)) {
    ...
    var prerequisiteSkipMessages = [
      ...
      "The web browser is blocking the Flash Player plugin on this web page.",
    ];
  ...
  test_source = test_source
    .replace(
      /^\s*\/\/ == BEGIN PREREQUISITE CHECKS ==[\S\s]*\/\/ == END PREREQUISITE CHECKS ==\n/m,
  ...
  var test_template = shouldBeBlocked
    ? function () {
        ...
        $.ajax({
          ...
          error: function (r, textStatus, errorThrown) {
            thisTest.SKIP(
              "This test could not finish executing, possibly due to a network problem. Re-run the test suite to run this test again.",
            );
          },
        });
      }
...
var credentialsExpect = function (testID, allowOrigin, allowCredentials, shouldBeBlocked) {
  var urlstr =
    "https://test.browseraudit.com/cors/allow-credentials/" +
    $.base64.encode(allowOrigin) +
    "/" +
    $.base64.encode(allowCredentials);
  ...
  var test_template = function () {
    ...
    $("<img>", { src: "http://browseraudit.com/set_referer" }).load(function () {
      $.get("/get_referer", function (referer) {
        if (referer === "") {
          thisTest.PASS("The Referer header was not sent in the request or was sent and autoupgraded to HTTPS.");
        } else {
...
var requestReferrerPolicy = function (testID, img_src, referrerPolicy, expected_result) {
  var imgSrc = img_src + "/set_referer";
  var reportData = {
...
var frameOptionsTest = function (testID, shouldBeBlocked, sourcePrefix, frameOptions, isIframe) {
  ...
  var frame_src = sourcePrefix + "/frameoptions/" + testID + "/" + defaultResult + "/" + frameOptionsBase64;
  ...
  var reportData = {
    ...
    frameHTTPHeader: "X-Frame-Options: " + frameOptions,
  };

  var test_template = function () {
    var thisTest = this;
    $(frameElement, { src: frame_src })
      .appendTo("div#sandbox")
      .load(function () {
