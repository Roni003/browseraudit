var browserAuditTestFramework = (function () {
    ...
    if (thisNode.nodeType === "category") {
      ...
      var testTimer = setInterval(function () {
        markHierarchyNode(
          ...
          thisNode.id,
          testStartTime,
          ["skip", "The test timed out (it did not finish executing within 10 seconds)."],
          testTimer,
        );
      }, 10000);
    ...
    while (!nextNodeFound) {
      ...
      if (thisNode.nodeType === "category") {
        if (thisNode.children.length === 0) {
          if (options.hasOwnProperty("endCategory"))
            options.endCategory.call(thisNode, { pass: 0, warning: 0, critical: 0, skip: 0 });
  ...
  var publicFramework = {
    ...
    addCategory: function (categoryID, parentCategoryID, title, description) {
      ...
      if (categoryTrail.hasOwnProperty(categoryID))
        throw "A category with this ID has already been added to the test suite.";

      // The parent category ID must be null, or must have been added to the
      // test suite already
      if (parentCategoryID !== null && !categoryTrail.hasOwnProperty(parentCategoryID))
        throw "The parent category's ID must have been added as a category already, or the parent category ID must be null (indicating that this is a top-level test category).";
    ...
    addTest: function (testID, categoryID, title, behaviour, timeout, failureSeverity, testFunction) {
      ...
      if (!categoryTrail.hasOwnProperty(categoryID))
        throw "A category with the given ID must have been added to the test suite already.";
